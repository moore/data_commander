...
void initColumn ( Column *column, column_t *iterator_column ) {
  ...
  init_varInt64_iterator( &(iterator_column->iterator),
			  Column_read_values( column ),
			  0,
			  Column_length_values( column ) );
...
iterator_t* initIterator ( TableTile* messageData ) {
  ...
  // BUG: no way to return errors
  iterator->count =
    (size_t)read_varInt64_vector_count(
      Column_read_values( indexColumn ),
      0,
      Column_length_values( indexColumn ) );
...
void updateColumn ( column_t *column, uint32_t index ) {
  ...
  // BUG: can't do anything with return value
  varInt64_get_next( &(column->iterator), &read_value );

  column->value = column->value
    + column->valueFactor * (double)read_value;
}
